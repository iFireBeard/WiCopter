...
void loop()
{
  ...
  // For more see http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles which has additional links.
    ...
    if ( Serial.available() ) {
      ...
      byte motor = (int)controll[0] - 48;
      String speedUnConverted = "";
      for (byte i = 1; i <= controll.length(); i++) {
        speedUnConverted += controll[i];
      }
      short unsigned int speed = speedUnConverted.toInt();

      switch (motor) {
        case 1: updateMotorsSpeed(speed,-1,-1,-1);   if (SerialDebug) { Serial.print("Motor1 Done"); } break;
      ...
      if(SerialDebug) {
        Serial.print("motor(");Serial.print(motor);Serial.print(")");Serial.print(" speed=");Serial.print(speed);
        Serial.println("");
      }
    }
...
void updateMotorsSpeed(int motor1, int motor2, int motor3, int motor4) {
  if (motor1 > MaxSpeed) motor1 = MaxSpeed;
  if (motor2 > MaxSpeed) motor2 = MaxSpeed;
  if (motor3 > MaxSpeed) motor3 = MaxSpeed;
  if (motor4 > MaxSpeed) motor4 = MaxSpeed;
  
  if (motor1 > 0) {
    motorsSpeed[0] = motor1;
  }
  if (motor2 > 0) {
    motorsSpeed[1] = motor2;
  }
  if (motor3 > 0) {
    motorsSpeed[2] = motor3;
  }
  if (motor4 > 0) {
    motorsSpeed[3] = motor4;
  }
}
